...
* If the event is in __terminal__ state, delete it. √
* If the `--pending` flag is set and the event is in a `PENDING` state, delete
  it. √
* If the `--processing` flag is set and the event is in a `PROCESSING` state,
  delete it. √

* Delete any Kubernetes resources associated with the event. √
  * Pods √
  * Secrets √
...
* If the event is in a `PENDING` state, update its state to `CANCELED`. √
* If the `--processing` flag is set and the event is in a `PROCESSING` state,
  update its state to `ABORTED`. √
  * For each worker, if the worker is in a `PENDING` state, update its state to
    `CANCELED`. √
  * For each worker, if the worker is in a `RUNNING` state, update its state to
    `ABORTED`. √
  ...
  * Workers already in a __terminal__ state should be unaffected. √
* Delete any Kubernetes resources associated with the event, _except for the
  event secret_. This is where the payload is stored and we may need that in the
  future if someone opts to clone the event.
  * Pods
  * Secrets
  * Persistent volume claims
...
* If the `--running` flag is set and the worker is in a `RUNNING` state, update
  its state to `ABORTED`.
  * For each job, if the job is in a `RUNNING` state, update its state to
    `ABORTED`.
  * Jobs already in a __terminal__ state should be unaffected.
* Determine how this state change affects overall event state.
* Delete any Kubernetes resources associated with the worker.
  * Pods
  * Secrets
  * Persistent volume claims
