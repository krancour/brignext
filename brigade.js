...
const localPath = "/workspaces/brigade-sdk-for-go";

// Run unit tests
function testUnit() {
  var job = new Job("test-unit", img);
...
function runSuite(e, p) {
  ...
  return Promise.all([
    runTests(e, p, testUnit).catch((err) => { return err }),
    runTests(e, p, lint).catch((err) => { return err }),
  ]).then((values) => {
