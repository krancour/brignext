...
type Service interface {
	...
	CreateUserSession(context.Context) (authx.UserSessionAuthDetails, error)
	Authenticate(
		ctx context.Context,
		oauth2State string,
		oidcCode string,
	) error
	GetByToken(context.Context, string) (authx.Session, error)
...
func (s *service) CreateRootSession(
	...
	session := authx.NewRootSession(token.Value)
	now := time.Now()
	session.Created = &now
...
func (s *service) CreateUserSession(
	ctx context.Context,
) (authx.UserSessionAuthDetails, error) {
	userSessionAuthDetails := authx.UserSessionAuthDetails{
		OAuth2State: crypto.NewToken(30),
		Token:       crypto.NewToken(256),
	}
	session := authx.NewUserSession(
...
func (s *service) Authenticate(
	...
	if err != nil {
		if _, ok := errors.Cause(err).(*core.ErrNotFound); ok {
			...
			if count == 0 {
				user.UserRoles = []authx.Role{
					authx.RoleAdmin(),
					authx.RoleProjectCreator(),
					authx.RoleReader(),
				}
			}
